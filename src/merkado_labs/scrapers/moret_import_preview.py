"""Moret Real Estate offline import preview and controlled import-from-file.

Makes no website requests. Import reparses local cache HTML only, uploads private
evidence when requested, and writes through the source-neutral importer.
"""
...
EXPECTED_CATALOG_CHECKSUM = (
    "386cbe6538899e12b6a7c78665b84d421c1745150c6e1271ca51adf8841505b0"
)
EXPECTED_LISTING_COUNT = 71
EXPECTED_INSERTS = 66
EXPECTED_UPDATES = 5
...
def load_moret_catalog(path: Path) -> dict[str, Any]:
...
def catalog_is_complete(payload: dict[str, Any]) -> bool:
    return payload.get("complete_catalog") is True
...
def listing_dict_to_snapshot(item: dict[str, Any]) -> AdapterListingSnapshot:
    ...
    return AdapterListingSnapshot(
        ...
        adapter_version=str(item.get("adapter_version") or "0.2.0"),
        raw_payload={
            "preview": True,
            "external_id": item.get("external_id"),
            "from_price": item.get("from_price"),
            "price_period": item.get("price_period"),
            "price_trusted": item.get("price_trusted"),
...
def load_existing_moret_rows(client: Any) -> list[dict[str, Any]]:
...
def reconcile_moret_catalog(
    *,
    ...
    """Compare catalog listings with current Labs Moret rows (in memory)."""
    ...
    result.notes.append(
        "First complete catalog establishes the baseline; prior bounded sample "
        "is not treated as an absence history."
    )
    ...
    if catalog.get("adapter_version") != "0.2.0":
        result.failed = True
        result.failure_reasons.append(
...
def import_moret_catalog_from_file(
    *,
    input_path: Path,
    cache_dir: Path,
    expected_checksum: str = EXPECTED_CATALOG_CHECKSUM,
    ...
    """Import the verified Moret complete catalog from local JSON + cache HTML only."""
    ...
    preview = reconcile_moret_catalog(
        catalog=catalog,
        existing_rows=load_existing_moret_rows(labs_client),
        expected_checksum=expected_checksum,
        project_ref=project_ref,
    )
    if preview.failed:
        raise RuntimeError(
            "Moret import refused: "
            + "; ".join(preview.failure_reasons or ["preview_failed"])
        )
    if preview.listing_count != EXPECTED_LISTING_COUNT:
        raise RuntimeError(
            f"Moret import refused: expected {EXPECTED_LISTING_COUNT} listings, "
            f"got {preview.listing_count}"
        )
    if (
        preview.inserts != EXPECTED_INSERTS
        or preview.updates != EXPECTED_UPDATES
        or preview.absent_from_catalog != 0
    ):
        raise RuntimeError(
            "Moret import refused: reconciliation drift "
            f"(insert={preview.inserts}, update={preview.updates}, "
            f"absent={preview.absent_from_catalog}); re-run preview before writing"
        )
    if preview.proposed_missing_events or preview.proposed_removed_events:
        raise RuntimeError(
            "Moret import refused: unexpected missing/removal transitions in preview"
        )
    if preview.identity_conflicts:
        raise RuntimeError("Moret import refused: identity conflicts in preview")
    ...
    for item in listings:
        ...
        snap = adapter.parse_listing_html(
            ...
            bilingual_aliases=item.get("bilingual_aliases"),
        )
    ...
    if len(set(external_ids)) != EXPECTED_LISTING_COUNT:
        raise RuntimeError(
            f"Moret import refused: parsed external IDs are not "
