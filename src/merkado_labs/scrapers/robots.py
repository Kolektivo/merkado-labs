"""Robots.txt policy helpers for bounded direct-source adapters."""
...
def check_robots(
    ...
    *,
    ...
    """Fetch robots.txt and evaluate whether the URL may be fetched.

    Returns can_fetch=None when robots.txt cannot be retrieved or parsed safely.
    Missing robots is never treated as blanket permission.
    """
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        return RobotsDecision(
            ...
            notes="URL is not a public http(s) URL",
        )
    ...
    try:
        ...
        if error.code in {401, 403}:
            return RobotsDecision(
                ...
                can_fetch=False,
                crawl_delay_seconds=None,
                notes="robots.txt access denied; fetch blocked",
            )
        if error.code == 404:
            return RobotsDecision(
                domain=domain,
                ...
                notes=(
                    ...
                    "require explicit adapter review before fetching."
                ),
            )
        return RobotsDecision(
            domain=domain,
    ...
    return RobotsDecision(
        ...
        notes="Evaluated against MerkadoLabs-PropertyAdapter user agent",
    )
