...
export async function mintOfferFor(reference: string): Promise<AutoMintResult> {
  ...
  if (receipt.status !== "success") {
        ...
        events: [
          {
            ...
            detail: "We could not prepare this listing yet. An Admin can retry the mint.",
            actor: "System",
          },
          ...current.events,
        ],
    ...
    return { status: "pending", txHash, reason: "The mint transaction reverted. An Admin can retry the mint." };
  }
...
/**
 * Mints every approved offer that still needs minting. Used only from
 * authenticated Admin flows; each offer retains its lease and receipt recovery.
 */
  ...
  for (const offer of pending) {
    try {
      ...
      results.push({
        reference: offer.reference,
        status: "pending",
        reason: error instanceof Error ? error.message : "The mint could not be completed.",
      });
    }
  }
  revalidate();
  return {
    minted: results.filter((result) => result.status === "confirmed").length,
    results,
  };
}
