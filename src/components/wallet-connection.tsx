...
import { ensureOptimismMainnet } from "@/lib/pay/wallet-adapter";
import { truncateHash } from "@/lib/rent-advance/ids";
import { OP_MAINNET_CHAIN_ID, OP_MAINNET_NETWORK_LABEL } from "@/lib/pay/networks";
...
export function WalletConnection({
  ...
  const handleSwitch = async () => {
    ...
    setSwitching(true);
    try {
      await ensureOptimismMainnet(wallet, true);
    } catch (err) {
  ...
  const statusText = wallet.isConnected
    ? `Connected to ${shortAddress}${
        onOptimismMainnet ? ` on ${OP_MAINNET_NETWORK_LABEL}` : " on another network"
      }`
    : wallet.connecting
      ? "Opening wallet…"
        ...
        <Popover>
          ...
          <PopoverContent align="start" className="w-80">
            ...
            {!onOptimismMainnet ? (
              ...
              >
                {switching ? "Switching…" : `Switch to ${OP_MAINNET_NETWORK_LABEL}`}
              </Button>
            ) : null}
