-- Atomic native-listing image removal.
-- Keeps image ordering, cover selection, public projection, eligibility, and
-- the immutable Passport event in one database transaction.
...
revoke all on function public.delete_native_listing_image(uuid, text)
  from public, anon, authenticated;

grant execute on function public.delete_native_listing_image(uuid, text)
  to service_role;

comment on function public.delete_native_listing_image is
  'Atomic manual listing image delete + reorder/cover + public projection + eligibility + Passport event. Service-role only.';
