-- English public presentation (Labs only).
-- Forward-only replacement of public_property_listings.
-- Prefers v5 English display_title / display_summary / display_description;
-- falls back to v4 then v3 for attributes. Source title/description preserved.
...
as
with retained as (
  -- Prefer latest succeeded/needs_review v5 proposal; fall back to v4 then v3.
...
select
  ...
  nullif(
    jsonb_strip_nulls(
      jsonb_build_object(
        -- Presentation layer is English when v5 display fields exist.
        'language', case
...
comment on view public.public_property_listings is
  'Public-safe effective listing projection. Prefers v5 English presentation (display_title, display_summary, display_description), falls back to v4/v3. Source title/description preserved. Never exposes raw proposals, evidence, confidence, tokens, or costs.';
