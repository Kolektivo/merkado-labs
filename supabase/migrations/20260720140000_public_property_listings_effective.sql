-- Public-effective listing projection for Browse / Property Passport.
-- Labs only (csaefdkpwukshtouyixg). Forward-only. Do not edit prior migrations.
--
-- Extends public_property_listings with consumer-safe effective fields while
-- keeping ai_enrichment_proposals service-role-only. The view runs as owner
-- (security_invoker=false) and projects only allowlisted final values — never
-- evidence snippets, confidence, tokens, costs, or raw proposal JSON.
...
as
with retained as (
  -- Current retained Terra-v3 proposal: matching checksum when contentful,
  -- otherwise the latest contentful proposal when the current checksum row is
  -- skipped_unchanged (semantic input unchanged; prior applied attrs still valid).
  ...
  join public.ai_enrichment_proposals a
   ...
   and a.prompt_version = 'listing_enrichment_v3'
   and a.schema_version = 'listing_enrichment_schema_v3'
   and a.status in ('succeeded', 'needs_review')
   and not exists (
     select 1
     from public.ai_enrichment_proposals failed
     where failed.property_listing_id = pl.id
       and failed.prompt_version = 'listing_enrichment_v3'
       and failed.schema_version = 'listing_enrichment_schema_v3'
       and failed.input_checksum = pl.enrichment_last_input_checksum
       and failed.status in ('failed', 'invalid_output')
   )
   and (
     a.input_checksum = pl.enrichment_last_input_checksum
     or exists (
       select 1
       from public.ai_enrichment_proposals skipped
       where skipped.property_listing_id = pl.id
         and skipped.prompt_version = 'listing_enrichment_v3'
         and skipped.schema_version = 'listing_enrichment_schema_v3'
         and skipped.input_checksum = pl.enrichment_last_input_checksum
         and skipped.status = 'skipped_unchanged'
     )
   )
  order by
    pl.id,
    case
      when a.input_checksum = pl.enrichment_last_input_checksum then 0
      else 1
    end,
    a.generated_at desc
),
  ...
  select
    ...
    (
      ...
      where d->>'key' = 'neighbourhood_candidate'
        ...
        and d->>'final_status' in ('needs_attention', 'auto_applied')
...
select
  ...
  coalesce(
    nullif(trim(p.source_property_type), ''),
    nullif(trim(p.ai_property_type), '')
  ) as effective_property_type,
  coalesce(p.public_attributes, '[]'::jsonb) as public_attributes,
  p.effective_summary
from projected p;

comment on view public.public_property_listings is
  'Public-safe effective listing projection. security_invoker=false so anon can read without grants on property_listings or AI proposal tables. Exposes only final effective values — never raw proposals, evidence, confidence, tokens, or costs.';
