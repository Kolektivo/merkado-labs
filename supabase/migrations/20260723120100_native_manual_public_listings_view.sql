-- Origin-aware public_property_listings (scraped + Labs admin manual).
-- Recreates the bilingual public view with left join to sources and
-- manual eligibility (no source_url required). Labs only.
...
projected as (
  select
    ...
    coalesce(pl.listing_origin, 'scraped') as listing_origin,
    pl.real_estate_type,
    pl.contact_name,
    pl.contact_method,
    pl.contact_value,
    ...
    pl.owner_attributes,
    case
      when pl.listing_origin = 'manual' then 'merkado_native'
      else ps.source_key
    end as source_key,
    case
      when pl.listing_origin = 'manual' then 'User provided'
      else ps.display_name
  ...
  from public.property_listings pl
  left join public.property_sources ps on ps.id = pl.property_source_id
  ...
  where pl.public_eligible = true
    ...
    and (
      (
        coalesce(pl.listing_origin, 'scraped') = 'scraped'
      ...
      or (
        pl.listing_origin = 'manual'
        and pl.primary_image_url is not null
        and length(trim(pl.primary_image_url)) > 0
        and pl.contact_method is not null
        and pl.contact_value is not null
        and length(trim(pl.contact_value)) > 0
        and pl.title is not null
        and length(trim(pl.title)) > 0
        and pl.real_estate_type is not null
        and length(trim(pl.real_estate_type)) > 0
        and pl.listing_type in ('sale', 'rent')
      )
    )
)
select
  ...
  case
    when p.listing_origin = 'manual' then coalesce(
      nullif(trim(p.real_estate_type), ''),
  ...
  case
    when p.listing_origin = 'manual' then coalesce(
      nullif(trim(p.display_title), ''),
      nullif(trim(p.title), '')
    )
    else p.display_title
  end as display_title,
  case
    when p.listing_origin = 'manual' then coalesce(
      nullif(trim(p.display_summary), ''),
      nullif(trim(p.effective_summary), ''),
      nullif(trim(left(coalesce(p.description, ''), 280)), '')
    )
    else coalesce(
      nullif(trim(p.display_summary), ''),
      nullif(trim(p.effective_summary), '')
  ...
  case
    when p.listing_origin = 'manual'
      and not public._public_is_generic_neighbourhood(p.source_neighbourhood_raw)
      then 'user_provided'
    when not public._public_is_generic_neighbourhood(p.source_neighbourhood_raw)
      then 'source'
    when not public._public_is_generic_neighbourhood(p.map_neighbourhood_name)
  ...
  case
    when p.listing_origin = 'manual'
      and jsonb_typeof(p.owner_attributes) = 'array'
      and jsonb_array_length(p.owner_attributes) > 0
      then p.owner_attributes
    else coalesce(p.public_attributes, '[]'::jsonb)
  end as public_attributes,
  p.effective_summary,
  nullif(
    jsonb_strip_nulls(
      jsonb_build_object(
...
comment on view public.public_property_listings is
  'Public-safe effective listing projection for scraped and Labs admin manual origins. Manual rows show User provided provenance and never require a scraper source URL. English display fields remain SEO-canonical for scraped rows.';
