...
function receipt(logs: FabricatedLog[], chainId = 84532, status = "success") {
  return {
    chainId,
    status,
    transactionHash: TX_HASH,
...
test("a reverted receipt is rejected", () => {
  const result = verify.verifyOfferMintedReceipt(
    receipt([offerMintedLog()], 84532, "reverted"),
    MINTED_EXPECTED,
  );
  assert.equal(result.verified, false);
  assert.equal(result.status, "failed");
...
test("a missing transaction maps to the pending result", () => {
  ...
  assert.equal(pending.chainId, 84532);
