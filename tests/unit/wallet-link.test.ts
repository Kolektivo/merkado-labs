...
test("buildLinkMessage binds account, domain, chain and nonce", () => {
  ...
  assert.ok(message.includes("Optimism Mainnet (10)"), "message must bind the chain");
...
test("chainLabelFor resolves Optimism Mainnet by chain id", () => {
  assert.equal(chainLabelFor(10), "Optimism Mainnet");
  assert.equal(chainLabelFor(999), "Unsupported chain 999");
});
