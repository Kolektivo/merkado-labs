...
export default async function AgentPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Merkado Agent" description="Labs preview for request-based matching. No email delivery or billing is enabled." icon={Bot} /><Card><CardHeader><CardTitle>Test access</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">{entitlements.length ? `${entitlements.length} test entitlement${entitlements.length === 1 ? "" : "s"} recorded.` : "No test entitlements yet."}</p><div className="flex flex-wrap gap-2">{entitlements.map((item) => <Badge key={item.id} variant="outline">{item.status} · {item.deliveryChannel}</Badge>)}</div><AgentEntitlementControl requests={confirmed} /></CardContent></Card></div>;
}
