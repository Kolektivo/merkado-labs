...
test("enrichment scope metadata is exactly one v3 value each", () => {
  ...
  assert.deepEqual([...new Set(promptMatches)], ["listing_enrichment_v3"]);
  assert.deepEqual(
    [...new Set(schemaMatches)],
    ["listing_enrichment_schema_v3"],
  );
  assert.deepEqual([...new Set(policyMatches)], ["enrichment_policy_v3"]);

  assert.match(scope, /export const PROMPT_VERSION = "listing_enrichment_v3"/);
  assert.match(
    scope,
    /export const SCHEMA_VERSION = "listing_enrichment_schema_v3"/,
  );
  assert.match(scope, /export const POLICY_VERSION = "enrichment_policy_v3"/);
