...
purchase/deposit begins, automatic receipt waiting and server verification without manual status controls,
...
9. Pay: seeded request **XCG 3,222.00** / **1,800.00 USDC** on the selected
    network (**Optimism Mainnet** after Reset); `depositRent(tokenId,
...
## Manual Optimism Mainnet E2E (run only after activation approval)

This guide replaces the old mocked walkthrough. The flow is the live flow on
**Optimism Mainnet**; it works only after the contract is verified,
...
1. Confirm `NEXT_PUBLIC_MERKADO_CONTRACT_ADDRESS` is set to the verified
    Optimism Mainnet contract and `MERKADO_RPC_URL` points at
...
2. Open **Admin** at the bottom of the left nav. Click **Reset the book** and
   ...
   return as `funding` offers with **empty on-chain state** — approve them in
    Admin; each approved offer starts its server-side mint automatically. A **new chain-store epoch**
   starts so old on-chain facts are never reused.
 3. Do not fund wallets or execute Mainnet transactions until the Product Lead
    approves the controlled gas/USDC budget and hosted activation.
...
### 3. Create offer and Admin (automatic server mint)
...
- Open **Admin**. Open the new offer. Pick **Enrique** or **Luuk**, then
  click **Approve offer**. The backend automatically broadcasts `mintOffer` from the server-held mint key and
...
- Open the **Punda** studio. The card shows one whole-offer price and no
  fractional option.
- Connect the Reown/AppKit wallet (must be on Optimism Mainnet; wrong-network is
...
- Connect a wallet on a network other than Optimism Mainnet. Purchase and
...
- Would a holder think they can buy in today, or that collections are
  guaranteed?
- Is the live Optimism Mainnet demo easy to explain without sounding like
  production?

If the live Optimism Mainnet walkthrough looks correct, reply:
`Approved, commit and push.` This does not approve uncontrolled real
