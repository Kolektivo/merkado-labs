...
The Optimism Mainnet flow adds chain store tables — `ra_chain_epochs`,
...
## 5. Optimism Mainnet contract flow
...
- **Pending-transaction recovery.** A submitted purchase / deposit / claim
  ...
  client-supplied address; the client waits for the receipt and automatically
  re-verifies the stored hash until settled, never blind re-sends or manual
  status checks. Mint recovery pins to the offer's original contract at
...
- **Pay QR (informational).** The QR, **copy address**, and **copy amount**
 ...
 - **Configuration.** `NEXT_PUBLIC_MERKADO_CONTRACT_ADDRESS` is the deployed
  Optimism Mainnet contract address; empty shows a not-configured state.
  Server-only `MERKADO_RPC_URL` defaults to `https://mainnet.optimism.io`.
 - **Not activated.** Contract verification, applying the chain-store
  migration, controlled minter funding, hosted activation, and merging the PR
  are separate gates. Uncontrolled real funds stay blocked.
