...
| P0 | Merkado Pay | Live USDC rent deposit on **Optimism Mainnet** via `depositRent`; exact monthly amount, opaque payment id; English-only; no demo outcomes |
...
| Instrument | Transferable offer NFT (`MerkadoRentOfferV1`, ERC-721) on Optimism Mainnet. One NFT per approved listing, minted automatically by the backend mint key after independent Admin approval. Current token owner = holder. Not a public token market. |
...
| Crypto in this task | Real Optimism Mainnet flow; wallet Reown/AppKit (injected EIP-1193); server receipt verification; no mock provider, network selector, `PAYMENT_RAIL_MODE`, demo wallet, demo outcome menu, or demo hashes |
| Network | **Optimism Mainnet** (`op-mainnet`). Fixed; no network selector or testnet fallback. |
...
| Approve + send | Marketplace purchase and Merkado Pay use a **single primary action** that opens one dialog and runs Approve USDC → wait for the successful approval receipt → purchase/deposit → automatic receipt wait and server verification until settled (never a blind re-send). One button style; no separate Approve step on the page. |
...
| Global Reset | Admin **Reset** keeps the current Reset UX and approval-triggered minting, and resets the one shared demo book. Reset does **NOT** roll back the chain. |
| Pending transaction binding | A submitted purchase / deposit / claim tx hash is stored on the shared offer or payment request. The current user's linked wallet authorizes the action; the client automatically re-verifies the stored hash until settled, never blind re-sends or asks the user to check status. |
...
| Contract deployment | When and how `MerkadoRentOfferV1` is verified and activated on Optimism Mainnet | Live activation |
