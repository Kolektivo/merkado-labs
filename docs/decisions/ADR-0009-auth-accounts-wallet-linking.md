...
Global Reset remains an Admin operation with its current UX and
approval-triggered minting. It
resets the shared Labs book, starts a new chain epoch, preserves network and
contract configuration, and does not roll back Optimism Mainnet. Old transaction
...
- Real funds, production access, and hosted activation remain blocked.
